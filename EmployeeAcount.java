...
public class EmployeeAccount {
    ...
    public EmployeeAccount(String employeeID, String name, List<BankingCSV.User> existingUsers) {
        ...
        // STEP 1: Load checking accounts from checking_accounts.csv if not already loaded
        if (existingUsers.isEmpty()) {
            try {
                List<BankingCSV.User> loaded = BankingCSV.readCSV("checking_accounts.csv");
                existingUsers.addAll(loaded);
            } catch (IOException e) {
                System.out.println("Note: checking_accounts.csv not found yet, will be created.");
            }
        }
 
        // Find or create a BankingCSV.User for this employee
        ...
        if (this.bankingUser == null) {
            ...
            // Save the new user to checking_accounts.csv
            try {
                BankingCSV.writeCSV("checking_accounts.csv", existingUsers);
            } catch (IOException e) {
                System.out.println("Warning: Could not write checking_accounts.csv: " + e.getMessage());
            }
        }
